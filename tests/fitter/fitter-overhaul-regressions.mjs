...
assert.ok(!/\.pyfa-damage-profiles\s*\{[^}]*display\s*:\s*none/i.test(statsCss), "damage profile controls must remain visible");
...
assert.match(fittings, /className="pyfa-section pyfa-section-capacitor"/);
assert.match(fittings, /className="pyfa-section pyfa-section-targeting"/);
assert.match(fittings, /className="pyfa-damage-profiles" open/);
assert.match(fittings, /profile-deal/);
assert.match(fittings, /profile-tank/);
assert.match(fittings, /pyfa-damage-profile-advanced/);
assert.match(fittings, />DEAL profile</);
assert.match(fittings, />TANK profile</);
assert.match(statsCss, /pyfa-damage-profiles\{order:20/);
