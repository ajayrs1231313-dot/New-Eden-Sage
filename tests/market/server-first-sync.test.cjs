...
assert.match(modal, /modal\.Dict\.from_name\(REFRESH_GUARD_DICT_NAME, create_if_missing=True\)/, "public refreshes should use a distributed non-blocking lease");
assert.match(modal, /skip_if_exists=True/, "overlapping scheduler ticks must skip rather than queue another heavy refresh");
assert.match(modal, /skipReason\": \"refresh-in-progress\"/, "overlapping refreshes should report the explicit skip reason");
assert.match(modal, /max_containers=4/, "refresh wrappers need enough concurrency to observe the lease instead of queueing behind one container");
