...
assert.match(renderer, /Preparing retained LP intelligence/, "LP page should show a local preparing state while the worker runs");
assert.match(renderer, /let cancelled = false;[\s\S]*if \(cancelled\) return;[\s\S]*cancelled = true/, "renderer should ignore stale results after navigation/selection changes");
