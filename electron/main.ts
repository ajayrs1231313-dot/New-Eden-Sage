...
app.whenReady().then(() => {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const result = await dialog.showSaveDialog(window, {
        title:
          format === "json"
            ...
            : format === "chatgpt-radius"
              ? "Export full high-sec market data"
      ...
      if (format === "json") content = JSON.stringify(data, null, 2);
        ...
        if (!full)
          throw new Error(
            "No full high-sec dataset exists yet. Run a full high-sec pull first.",
          );
  ...
  ipcMain.handle("trade:radius-opportunities", async (_event, mode) =>
    findRadiusTrades(mode),
  );
  ipcMain.handle("trade:export-top1000", async () => {
    if (!window) return null;
    const analysis = await findRadiusTrades("top1000");
    const exportStamp = new Date().toISOString().replace(/[:.]/g, "-");
    const result = await dialog.showSaveDialog(window, {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (input.mode === "all" || input.mode === "contracts") {
      ...
      return { summaries: marketSummaryHeaders(summaries), storage };
    },
  );
  createWindow();
});
